        ...
        case FRAME_TYPE_JOIN_ACCEPT:
            ...
            if( LORAMAC_CRYPTO_SUCCESS == macCryptoStatus )
            {
                ...
                MacCtx.NvmCtx->NetworkActivation = ACTIVATION_TYPE_OTAA;
								...
								PRINTF("\r\n netID: %02X",MacCtx.NvmCtx->NetID);
								PRINTF("\r\n addr: %02X",MacCtx.NvmCtx->DevAddr); 
								PRINTF("\r\n Rx1DrOffset: %02X",MacCtx.NvmCtx->MacParams.Rx1DrOffset); 
								PRINTF("\r\n Rx2Channel.Datarate: %02X",MacCtx.NvmCtx->MacParams.Rx2Channel.Datarate); 
								PRINTF("\r\n RxCChannel.Datarate: %02X",MacCtx.NvmCtx->MacParams.RxCChannel.Datarate); 
								PRINTF("\r\n ReceiveDelay1: %02X",MacCtx.NvmCtx->MacParams.ReceiveDelay1); 
								PRINTF("\r\n Payload: %02X",applyCFList.Payload); 
...
bool LoRaMacIsBusy( void )
{
    ...
    LORAMAC_RX_ABORT      = 0x00000080,
	*/
	switch(MacCtx.MacState){
		case LORAMAC_IDLE: 
			PRINTF("LORAMAC_IDLE");
		  break;
		case LORAMAC_STOPPED:
			PRINTF("LORAMAC_STOPPED");
		  break;
		case LORAMAC_TX_RUNNING:
			PRINTF("LORAMAC_TX_RUNNING");
		  break;
		case LORAMAC_RX: 
			PRINTF("LORAMAC_RX");
		  break;
		case LORAMAC_ACK_RETRY:
			PRINTF("LORAMAC_ACK_RETRY");
		  break;
		case LORAMAC_TX_DELAYED:
			PRINTF("LORAMAC_TX_DELAYED");
		  break;
		case LORAMAC_TX_CONFIG:
			PRINTF("LORAMAC_TX_CONFIG");
		  break;
		case LORAMAC_RX_ABORT:
			PRINTF("LORAMAC_RX_ABORT");
		  break;
		default:
			PRINTF("LORA_DEFAULT");
	}
...
LoRaMacStatus_t PrepareFrame( LoRaMacHeader_t* macHdr, LoRaMacFrameCtrl_t* fCtrl, uint8_t fPort, void* fBuffer, uint16_t fBufferSize )
{
    ...
    switch( macHdr->Bits.MType )
    {
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_UP:
            ...
            if( LORAMAC_CRYPTO_SUCCESS != LoRaMacCryptoGetFCntUp( &fCntUp ) )
            {
								PRINTF(" | LORAMAC_STATUS_FCNT_HANDLER_ERROR | ");
                return LORAMAC_STATUS_FCNT_HANDLER_ERROR;
            }
...
LoRaMacStatus_t SendFrameOnChannel( uint8_t channel )
{
		PRINTF(" | inside SendFrameOnChannel | ");
...
void LoRaMacInitializationReset(void){
    ...
    SX1276SetRadioEvents( &MacCtx.RadioEvents );
		SX1276SetLoRaBoardCallbacks();
		SX1276SetOpMode( RF_OPMODE_SLEEP );
		Radio.SetPublicNetwork( MacCtx.NvmCtx->PublicNetwork ); // SetModem is called in this function
		Radio.Sleep( );
...
{
	  PRINTF(" | inside LoRaMacMcpsRequest | ");
    GetPhyParams_t getPhy;
    PhyParam_t phyParam;
    LoRaMacStatus_t status = LORAMAC_STATUS_SERVICE_UNKNOWN;
    LoRaMacHeader_t macHdr;
